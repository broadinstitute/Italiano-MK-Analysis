...
class ResultsImage(ResultsDF):
    ...
    def analyze(self):
        ...
        if self.time is None:
            ...
            self.plot(ppp, 'line', tools.create_folder(self.path,'PPLTPct'))
            self.plot(am, 'line', tools.create_folder(self.path,'MKArea'))
            self.plot(ap, 'line', tools.create_folder(self.path,'PPLTArea'))

        y = x.reset_index()
        y = y.drop(['ImageNumber','FileName_phase','Metadata_Well_Site',
        'Metadata_Well_Site_Time','Metadata_Well','Metadata_Site','AreaOccupied_TotalArea_mk_filter',
        'AreaOccupied_TotalArea_pplt_filter'],axis=1)
        y = y.groupby('Metadata_Time').mean()
...
class ResultsObject(ResultsDF):
    ...
    def analyze(self):
        ...
        y = x.reset_index()
        y = y.drop(['Metadata_Well_Site','Metadata_Well_Site_Time_Obj','Metadata_Well','Metadata_Site',
        'ImageNumber','ObjectNumber','Location_Center_X','Location_Center_Y','AreaShape_Center_X',
        'AreaShape_Center_Y'],axis=1)
        y = y.groupby('Metadata_Time').mean()
        y.columns = [str(col) + '_' + self.object_name for col in y.columns]
